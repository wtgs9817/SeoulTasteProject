...
<script>
	...
	function requestData(start, end) {
		...
		var dataUrl = 'http://openapi.seoul.go.kr:8088/4d4474467a77746736306d745a6855/xml/LOCALDATA_072404/'; /* URL */


		var url = dataUrl + start + "/" + end + "/"; // 요청 URL 생성
		console.log(url + " [" + start + "~" + end + "]" + "데이터 검색중");
		...
		xhr.onreadystatechange = function () {
			if (this.readyState == xhr.DONE) {  // <== 정상적으로 준비되었을때
				if (xhr.status == 200 || xhr.status == 201) { // <== 호출 상태가 정상적일때
					...
					var restaurantStatusList = xmlobj.getElementsByTagName("DTLSTATENM");  // 영업여부
					...
					for (var i = 0; i < restaurantNamelist.length; i++) {
						if (restaurantNamelist[i].textContent.includes(keyword)) {
							...
							restaurantStatus = restaurantStatusList[i].textContent;
						}
					}
					//데이터 확인
					//console.log(restaurantName);
					...
					if (restaurantName !== "") {  //가게명이 제대로 받아졌다면
						keyword = address;
					...
					else {  //현재 요청받은 데이터 중에 가게명이 없을 때
						if(restaurantNamelist.length == 1000) {  //데이터가 요청한 만큼 다 받아졌을 때
							requestData(start + 1000, end + 1000);  //다음 데이터 추가 요청 (한번의 요청에 1000개 까지밖에 못받아옴)
						}
						else { // 1000개의 데이터를 요청했는데 받아진 데이터의 개수가 1000개 미만일 때 ( 모든 데이터를 받아왔을 때)
							//모든 데이터와 비교했는데도 찾는 데이터가 없을 때
							alert("검색 결과 없습니다.")
						}
					}
